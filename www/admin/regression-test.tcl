...
::xoexception::try {
  ...
  ::xorb::ServiceImplementation PerObjectImplementation \
      -implements ::template::AATreaty \
      -using {
	Delegate m1 -for ::template::servantProc;# per-instance!
	Delegate m2 -for {::template::ServantObj servantMethod} \
	    -per-object true \
	    -deprecated_p false \
	    -warn_p false \
	    -private_p false \
  ...
  # positive test
  #AA-2-LC-AdapterAA-2-LC-Adapter
  ::xo::cc user_id 1
  ns_write <pre>[::xo::cc user_id]</pre>
  ?+ {$i invoke}  [subst {
    Dispatching invocation call: [::xorb::ic virtualObject]->[::xorb::ic virtualCall]\
  ...
  CallerInterface mixin add ::xorb::Synchronizable
  CallerInterface sync -delete

  ? { db_0or1row contract_deleted \
	  [CallerInterface subst { select * 
	    from acs_sc_contracts
	    where contract_id = $object_id }]
  } 0 "Unstaging: test contract (id:[CallerInterface object_id]) was removed."

  CallerInterface mixin delete ::xorb::Synchronizable
  ...
  CalleeInterface mixin add ::xorb::Synchronizable
  CalleeInterface sync -delete
  
  ? { db_0or1row impl_deleted \
	  [CalleeInterface subst { select * 
	    from acs_sc_impls
	    where impl_id = $object_id }]
  } 0 "Staging: CalleeInterface (id:[CalleeInterface object_id]) was removed."
  
  CalleeInterface mixin delete ::xorb::Synchronizable
...
"
  ...
  global errorInfo 
  ns_write "Test run failed: $errorInfo"
  ns_write "Proceeding with cleanup ..."
} finally {
  if {[db_driverkey ""] eq "postgresql"} {
    set packageRoot [acs_package_root_dir xotcl-request-broker]
    set path $packageRoot/www/admin/regression-test-cleanup.sql
    db_source_sql_file $path
  }

  # reset Broker, i.e. stateful bindings
  ::XorbManager do ::xorb::manager::CRepository destroy
  ::XorbManager do ::xorb::manager::Repository \
      ::xorb::manager::CRepository -itemType ::xorb::ServiceContract
  ::XorbManager do ::xorb::manager::IRepository destroy
  ::XorbManager do ::xorb::manager::Repository \
      ::xorb::manager::IRepository -itemType ::xorb::ServiceImplementation
  ::XorbManager do ::xorb::manager::Broker init 

  # clear test object
  ::template::test destroy
} 