...
\lstinputlisting[breaklines=true,numbers=left,frame=single,basicstyle=
\footnotesize,firstline=6,lastline=10]{../examples/xosoap/example-10-soap-provider-init.tcl}
 
In view of our code base, we could then rewrite our service implementation the following way, using \objlink{::xorb::Delegate}.

\lstinputlisting[breaklines=true,numbers=left,frame=single,basicstyle=
\footnotesize,firstline=39,lastline=47]{../examples/xosoap/example-10-soap-provider-init.tcl}
...
define proxy methods in the literal sense of the word on ::xotcl::Object and its subclasses, \proclink{::xotcl::Object}{instproc}{glue}
...
Providing framework support for data type handling, especially in a cross-language and multi-protocol setup is an ingenious task. Nevertheless, framework designers and developers may revert to well documented experiences on these issues and do not have to start from scratch. By these documented experiences, we primarily refer to a set of design patterns, some of them to be found in \cite{zdun:2005b,sommerlad:1998,maetzel:1996}. For the very moment, we do not want to go into details of the framework implementation, rather provide a quick overview on, first, terminology and, second, facilities to be used by you, the developer.
...
As for terminology, there are just very few concepts to remember. If you recall the examples in specifying client proxies or service contracts (see Section \ref{sec:quickstart:xosoap}), the way you specified the signature of a proxy method or \objlink{::xorb::Abstract} always involved specific \href{http://media.wu-wien.ac.at/doc/tutorial.html#non-pos-args}{check options}. These might look familar: 
 ...
 \lstinputlisting[breaklines=true,numbers=left,frame=single,basicstyle=
\footnotesize,,firstline=17,lastline=17]{../examples/xosoap/example-01-soap-consumer.tcl}

     \lstinputlisting[breaklines=true,numbers=left,frame=single,basicstyle=
\footnotesize,firstline=10,lastline=13]{../examples/xosoap/example-07-soap-provider-init.tcl}
...
Referring to lines 17-21, we extended the method's record by an additional \href{http://media.wu-wien.ac.at/doc/tutorial.html#non-pos-args}{non-positional argument} and 
...
%
\lstinputlisting[breaklines=true,numbers=left,frame=single,basicstyle=\footnotesize,firstline=4,lastline=13]{../examples/xorb/example-01-call-abstraction.xotcl}
%
...
%
\lstinputlisting[breaklines=true,numbers=left,frame=single,basicstyle=\footnotesize,firstline=15,lastline=16]{../examples/xorb/example-01-call-abstraction.xotcl}
%
...
%
\lstinputlisting[breaklines=true,numbers=left,frame=single,basicstyle=\footnotesize,firstline=18,lastline=26]{../examples/xorb/example-01-call-abstraction.xotcl}
%
...
%
\lstinputlisting[breaklines=true,numbers=left,frame=single,basicstyle=\footnotesize,firstline=37,lastline=44]{../examples/xorb/example-01-call-abstraction.xotcl}
%
...
\footnotesize,caption=Shared scope,label=lst:quickstart:xosoap:9]{../examples/xosoap/example-09-soap-consumer.tcl}
     ...
     \lstinputlisting[breaklines=true,numbers=left,frame=single,basicstyle=
\footnotesize,label=lst:quickstart:xosoap:10]{../examples/xosoap/example-10-soap-provider-init.tcl}
